...
\DIFdelend \DIFaddbegin \DIFadd{For the use case of circuits and FPGAs which are relevant for Spade this can be a large problem. This problem is further discussed in section \ref{sec:MotivationIAandAA}.
}\DIFaddend 
...
\DIFadd{The implementation is then evaluated based on X example programs in Chapter \ref{cha:ImplementationEval} -- these programs are available in the Appendix \ref{app:Programs} and the }\href{https://github.com/FredTheDino/thesis-spade-lang}{\DIFadd{git-repository for this thesis}}\DIFadd{. Here the synthesis tool -- yosys in this thesis -- generates statistics that are collected before and after the changes to the Spade compiler -- including if the program type checked before or not. The information collected for each program with each version of the Spade compiler are:
    ...
    \DIFaddend \item \DIFdelbegin \DIFdel{Implement wordlength deduction in Spade in a seperate module
