...
The wordlength inference was also compared on 3 expressions: $(a - a) + (a - a) + (a - a)$, $(a - b) \cdot (b - a)$ and $a \cdot a \cdot b \cdot b \cdot c \cdot c$, for all of the 3 methods ''AA'', ''IA'' and ''AAIA''. All arguments had a wordlength of $5$ and the range of the resulting expression was noted in the table in Figure \ref{fig:CompareThings}.
...
\begin{figure}
  ...
  \caption{A table showing how different wordlength inference methods compare on simple mathematical expressions where all variables have the word length of 5}
...
The same experiment as outlined in the previous paragraph was also run when all variables had a range of $0..100$ instead of an exact wordlength. The results were noted in the table in Figure \ref{fig:CompareThingsPositive}
\begin{figure}
  \centering
  \begin{tabular}{l | c c c}
                                    & IA     & AA     & AAIA    \\
  ...
  \caption{A table showing how different wordlength inference methods compare on simple mathematical expressions where all variables have the range 0..100}
  \label{fig:CompareThingsPositive}
\end{figure}

In the table in Figure \ref{fig:CompareThings2} ''AAIA'' always produced the tightest ranges. The multiplication test on the third row shows an example expression where ''IA'' produces a tighter range than ''AA''.
