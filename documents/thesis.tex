\documentclass[msc,lith,english]{liuthesis}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Imports
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \usepackage[english]{babel}
\usepackage[utf8]{inputenc}
\usepackage[backend=biber,sorting=none,hyperref]{biblatex}
\usepackage{mathtools}
\usepackage{dsfont}
\usepackage{tikz}
\usetikzlibrary{topaths,calc,tikzmark}
\usepackage{algorithm2e}
\usepackage{pgfgantt}
\usepackage{marginnote}
\usepackage{marvosym}
\usepackage[marginpar]{todo}
\usepackage{pgfgantt}
\usepackage{textcomp}
\usepackage{subcaption}
\usepackage[T1]{fontenc}
\usepackage{inconsolata}
\usepackage{minted}
\usepackage{cprotect}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Settings
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\department{Institutionen för systemteknik}
\departmentenglish{Department of Electrical Engineering}
\departmentshort{ISY}

\supervisor{Frans Skarman}
\examiner{Oscar Gustafsson}
\titleenglish{Wordlength inference in the Spade HDL}
\subtitleenglish{Seven implementations of wordlength inference and one implementation that actually works}
\titleswedish{Ordlängdsinferans i Spade HDL}
\subtitleswedish{Sju olika implementationer av ordlängdsinferens och en implementation som faktiskt fungerar}
\thesissubject{Datateknik}

\publicationyear{2023}
% \currentyearthesisnumber{LiTH-ISY-EX--23/5624--SE}
\currentyearthesisnumber{5624}
\dateofpublication{2023-05-20}

\addbibresource{thesis.bib}

\renewcommand{\todomark}{TODO}

\newcommand{\hyperfootnote}[2]{\href{#2}{#1}\footnote{\href{#2}{#2}}}
\newcommand{\say}[1]{``#1''}

\author{Edvard Thörnros}

\begin{document}

% introduction.tex
\include{introduction}

% background.tex
\include{background}

% related_work.tex
\include{related_work}

% method.tex
\include{method}

% results.tex
\include{results}

% discussion.tex
\include{discussion}

% conclusion.tex
\include{conclusion}

\clearpage

\printbibliography

\appendix
% appendix_version_and_hashes.tex
\include{appendix_version_and_hashes}

\todos

\end{document}
